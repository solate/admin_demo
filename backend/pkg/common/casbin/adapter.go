...
// 	client, err := generated.NewClient(context.Background(), dataSourceName)
// 	if err != nil {
// 		return nil, err
// 	}
// 	a := &Adapter{
...
func (a *Adapter) LoadPolicy(model model.Model) error {
	policies, err := a.client.CasbinRule.Query().Order(generated.Asc("id")).All(a.ctx)
	if err != nil {
		return err
	}
...
func (a *Adapter) SavePolicy(model model.Model) error {
	return a.WithTx(func(tx *generated.Tx) error {
		if _, err := tx.CasbinRule.Delete().Exec(a.ctx); err != nil {
			return err
		}
		lines := make([]*generated.CasbinRuleCreate, 0)
...
func (a *Adapter) WithTx(fn func(tx *generated.Tx) error) error {
...
func loadPolicyLine(line *generated.CasbinRule, model model.Model) {
...
func (a *Adapter) toInstance(ptype string, rule []string) *generated.CasbinRule {
	instance := &generated.CasbinRule{}
...
func (a *Adapter) savePolicyLine(tx *generated.Tx, ptype string, rule []string) *generated.CasbinRuleCreate {
...
func (a *Adapter) UpdateFilteredPolicies(sec string, ptype string, newPolicies [][]string, fieldIndex int, fieldValues ...string) ([][]string, error) {
	...
	err := a.WithTx(func(tx *generated.Tx) error {
		cond := make([]predicate.CasbinRule, 0)
		cond = append(cond, casbinrule.PtypeEQ(ptype))
		if fieldIndex <= 0 && 0 < fieldIndex+len(fieldValues) && len(fieldValues[0-fieldIndex]) > 0 {
			cond = append(cond, casbinrule.V0EQ(fieldValues[0-fieldIndex]))
...
func (a *Adapter) createPolicies(tx *generated.Tx, ptype string, policies [][]string) error {
	lines := make([]*generated.CasbinRuleCreate, 0)
	for _, policy := range policies {
		lines = append(lines, a.savePolicyLine(tx, ptype, policy))
	}
...
func CasbinRuleToStringArray(rule *generated.CasbinRule) []string {
	arr := make([]string, 0)
	if rule.V0 != "" {
